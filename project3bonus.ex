defmodule Project3bonus do
  ...
  def main(args) do
    cond do
     (Enum.count(args) == 3) -> 
      ...
      numFailures = Enum.at(args, 2)|>String.to_integer()
      
      initiatePastry(numNodes,numRequests, numFailures)
      infiniteLoop()

    (Enum.count(args) == 2) ->
      numNodes=Enum.at(args, 0)|>String.to_integer()
      numRequests=Enum.at(args, 1)|>String.to_integer()
      numFailures = round(numNodes/100)

      initiatePastry(numNodes,numRequests, numFailures)
      infiniteLoop()

    true ->
      IO.puts "Please provide atleast 2 or 3 arguments"
      System.halt(1)  
    end
  end 
  ...
  def initiatePastry(numNodes, numRequests, numFailures) do
    ...
    if(numFailures > numNodes) do
      IO.puts "Failed nodes can not be more than total number of nodes"
      System.halt(1)
    end
    ...
    :ets.insert(:etsTable, {"numFailures",numFailures})
  ...
  def handle_cast({:DistributedHashTablesFormed},state) do
    ...
    if(numberOfNodesJoined == numInitialPhase) do
      if(numberOfNodesJoined >= numNodes) do
        GenServer.cast(:master,{:StartFailing})
      else
  ...
  def handle_cast({:SendMsg,fromID, destinationID, hops, tableRows, nodeIDSpace},state) do
    ...
    if myID == destinationID do
      ...
      cond do
        ...
        diff = nodeIDSpace + 10
        nearest = -1
        {nearest, diff} = getNewNearestAndDifference(table,shl, destinationID, diff, nearest, 0)
  ...
  def handle_cast({:MessageReceivedAtDestination,fromID,destinationID,hops},state) do
    ...
    [{_, numRequests}]=:ets.lookup(:etsTable, "numRequests")
    [{_, numNodes}]=:ets.lookup(:etsTable, "numNodes")
    [{_, numFailures}]=:ets.lookup(:etsTable, "numFailures")
  ...
  def handle_cast({:StartFailing}, state) do
    [{_, numFailures}]=:ets.lookup(:etsTable, "numFailures")
    [{_, cryptoHashes}]=:ets.lookup(:etsTable, "cryptoHashes")

    if(numFailures > 0) do
      Enum.each(0..numFailures - 1, fn(k) ->
        nodeID = Enum.fetch!(cryptoHashes,k)
        nameAtom= nodeID |>Integer.to_string|>String.to_atom
        GenServer.cast(nameAtom,{:Fail})
      end)
    end  
    :timer.sleep(10)
  ...
  def handle_cast({:Fail}, state) do
    ...
    Enum.each(0..numNodes-1, fn(x) -> 
      nodeID = Enum.fetch!(cryptoHashes,x)
      nameAtom= nodeID |>Integer.to_string|>String.to_atom
      GenServer.cast(nameAtom,{:RemoveFromDistributedHashtable , myID})
     end)
 
    nameAtom= myID |>Integer.to_string|>String.to_atom
    GenServer.stop(nameAtom, :normal)

    {:noreply, state}
  end

  def handle_cast({:RemoveFromDistributedHashtable, theID}, state) do
    {myID,smallerLeaf,largerLeaf, table, numOfBack}=state
    [{_, tableRows}]=:ets.lookup(:etsTable, "tableRows")
    if((theID > myID) && Enum.member?(largerLeaf,theID)==true) do
      largerLeaf = largerLeaf -- [theID]
      if(length(largerLeaf) > 0) do
        nameAtom = Enum.max(largerLeaf)|>Integer.to_string|>String.to_atom
        GenServer.cast(nameAtom, {:RemoveFromLeafSet, theID, myID})
      end
    end

    if((theID < myID) && Enum.member?(smallerLeaf,theID)==true) do
      smallerLeaf = smallerLeaf --[theID]
      if(length(smallerLeaf) > 0) do
        nameAtom = Enum.min(smallerLeaf)|>Integer.to_string|>String.to_atom
        GenServer.cast(nameAtom, {:RemoveFromLeafSet, theID, myID})
      end
    end
    shl= commonPrefixLength(convertToBase4String(myID,tableRows),convertToBase4String(theID,tableRows))
    
    if(table[shl][convertToBase4String(theID, tableRows)|>String.at(shl)|>String.to_integer] == theID) do
      col=convertToBase4String(theID, tableRows) |>String.at(shl)|>String.to_integer
      table = put_in(table[shl][col], -1) 
      table = callRequestInTableRecursive(shl, theID, table,myID,tableRows, 0)
    end
  ...
  def callRequestInTableRecursive(shl, theID, table,myID,tableRows, counter) when counter ==4 do
    table
  end

  def callRequestInTableRecursive(shl, theID, table,myID,tableRows, counter) do
    if((table[shl][counter] != myID) && (table[shl][counter] != theID) && table[shl][counter] !=-1) do
      nameAtom = table[shl][counter]|>Integer.to_string|>String.to_atom
      column = convertToBase4String(theID, tableRows)|>String.at(shl)|>String.to_integer
      GenServer.cast(nameAtom, {:CheckTableForRecovery, shl, column, myID})
    end
    callRequestInTableRecursive(shl, theID, table,myID,tableRows, counter + 1)
  end

  def handle_cast({:RemoveFromLeafSet, theID, sender},state) do
    {myID,smallerLeaf,largerLeaf, table, numOfBack} = state
    temp = smallerLeaf ++ largerLeaf -- [theID]
    nameAtom = sender|>Integer.to_string|>String.to_atom
    GenServer.cast(nameAtom, {:RecoverLeafSet, temp, theID})

    {:noreply, state}
  end

  def handle_cast({:RecoverLeafSet, newList, theID},state) do
    {myID,smallerLeaf,largerLeaf, table, numOfBack} = state
    
    Enum.each(newList, fn(i) -> 
      if((i > myID) && (Enum.member?(largerLeaf,i)==false)) do
        if(length(largerLeaf) < 4) do
          largerLeaf = largerLeaf ++ [i]
        else
          if(i < Enum.max(largerLeaf)) do
      ...
      if((i < myID) && (Enum.member?(smallerLeaf,i)==false)) do
        if(length(smallerLeaf) < 4) do
          smallerLeaf = smallerLeaf ++ [i]
        else
          if(i < Enum.min(smallerLeaf)) do
  ...
  def handle_cast({:CheckTableForRecovery, shl, column, sender},state) do

    {myID,smallerLeaf,largerLeaf, table, numOfBack} = state

    if(table[shl][column] == -1) do
      nameAtom = sender|>Integer.to_string|>String.to_atom
      
      GenServer.cast(nameAtom, {:RecoverDistributedHashtable, shl, column, table[shl][column]})
    end

    {:noreply, state}
  end

  def handle_cast({:RecoverDistributedHashtable, row, column, newID },state) do

    {myID,smallerLeaf,largerLeaf, table, numOfBack} = state

    if(table[row][column] == -1) do
      table = put_in(table[row][column], newID) 
    end
    state = {myID,smallerLeaf,largerLeaf, table, numOfBack}
  ...
  def getNewNearestAndDifference(table,shl, destinationID, diff, nearest, counter) when counter == 4 do
    {nearest, diff}
  end

  def getNewNearestAndDifference(table,shl, destinationID, diff, nearest, counter) do
    if((table[shl][counter] != -1) && (abs(table[shl][counter] - destinationID) < diff )) do
      diff = abs(table[shl][counter] - destinationID)
      nearest = table[shl][counter]
    end
    getNewNearestAndDifference(table, shl, destinationID, diff, nearest, counter + 1)
  end
  ...
  def commonPrefixLength(str1,str2) do
    count=0
